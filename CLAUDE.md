...
- **Cantidad de jugadores contra bots (sept 2026):** en la pantalla «Tu nombre» de `/jugar/bots`, un selector «Jugadores: 3 · 4» (vos y 2 o 3 bots), por defecto 4 y recordado en `localStorage` como el nombre, el personaje y el color (`identity.ts`, `savePlayerCount`). El tablero toma los primeros N asientos (`SEATS` en `board.js`) y esconde el 4.º puesto, igual que en línea; el mapa es el mismo. En línea ya se elegía de hecho según cuántos se sumen al lobby (3 o 4); la mesa local sigue en 4. «Nuevo mapa» respeta la cantidad y el interruptor «Caos» (reconstruye la sesión con las mismas opciones).
