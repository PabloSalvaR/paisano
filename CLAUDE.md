...
- **Banner de recursos y jugadores (simulación en el cliente):** banner abajo al centro con el color del jugador 1 (rojo, `VIEWER = 0`): avatar, nombre, puntos de victoria (estrella, siempre 0 por ahora) y 5 tarjetas (icono SVG + cantidad). A la izquierda, bajo el título, los puestos de los 4 jugadores simulados (Tomás rojo, Lucía azul, Mateo naranja, Sofía blanco) con avatar, nombre, PV y cantidad de cartas; el turno rota tras cada tirada y resalta su puesto. Todos arrancan con 0 recursos. Al salir un número, cobra quien tiene poblado (1) o ciudad (2) en un vértice de la casilla (`t.owners`, calculado en `placePieces`); un icono con insignia del color del dueño vuela de la casilla a su tarjeta (si es el jugador 1) o a su puesto, con un `+n` de ~2 s. La cámara se desplaza (`setViewOffset`) para centrar el tablero sobre el banner. Es provisorio: con el motor de reglas todo vendrá del servidor. El ladrón aún no bloquea la producción.
