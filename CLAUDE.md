...
- **Botón de turno y carta de desarrollo (sept 2026):** "Tirar dados" y "Terminar turno" pasaron a ser un solo botón que cambia según la fase (ver Referencia visual). Sobre el banner de recursos aparece un botón con una carta (`#btnDev`) solo en fase `main` y si la mano alcanza para `config.costs.developmentCard`; al tocarlo abre el modal ✓/✕ pegado al botón (`placeConfirm`). **Pendiente:** el motor aún no tiene `buyDevCard` (fase 3), así que el ✓ solo muestra un aviso; conectar cuando exista el comando. El texto de estado se oculta mientras hay una confirmación abierta.
