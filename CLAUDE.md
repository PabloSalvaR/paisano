...
- Cliente conectado por `GameSession`: **menú de inicio en `/`** (bots, online, mesa local, con la versión de `package.json` y el lema «Piedra y camino»), tablero sin red en `/jugar/bots` y `/jugar/local`, con reproducción de eventos y los bots esperando 2 s antes de cada acción.
