...
- **Reparto de números (sept 2026):** primero se usó solo azar + reintento; después, a pedido del desarrollador, **por defecto van en serie** (espiral en el orden de las letras A–R del juego original, `GameConfig.numberPlacement: 'spiral'`, que es también el valor del juego base) y el azar pasó a ser el **modo «Caos»** (`'random'`). Las letras no se muestran en las fichas: quedan detrás de la lógica y se explican en el Reglamento (sección «El mapa»). Terrenos y puertos siguen siempre al azar. **Interfaz:** un interruptor «Caos: números al azar» (con los colores de los botones y selectores) en la pantalla «Tu nombre» de `/jugar/bots`, **siempre apagado al entrar** (a propósito no se recuerda en `localStorage`: se juega en serie salvo que se pida el caos para esa partida). La mesa local y las salas online no tienen el check y juegan en serie (valor por defecto del motor). Código: `spiralOrder`/`spiralNumbers` en `engine/map.ts`.
