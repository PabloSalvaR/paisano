...
5. **Cliente jugable:** partida local, contra bots y online. — local y contra bots hechos; online (6b) en curso
...
- Verificado con Chrome (clics reales, escritorio y celular): construir con ✓/✕, dados y producción, 7 con descarte y robo, comercio, bots colocando y construyendo.

**En curso: 6b, partida online con amigos.** `RemoteSession` con polling, pantalla de sala (`/sala/CODIGO`, «Hola, presentate compañero», lobby con «Sumar bot» y «Empezar»), token en `localStorage`, botón «Jugar online».

**Siguiente (orden acordado)**
1. Cuenta gratis de **Upstash Redis** (el desarrollador la crea; URL y token en variables de entorno, nunca en el repo; **avisarle antes de necesitarla**), `UpstashStore` y prueba en Vercel.
...
**Sin probar a mano:** sonido de aterrizaje y de comercio, ritmo de las pausas de los bots (650 ms por pieza, 800 el ladrón, 450 el cambio de turno), animaciones con la pestaña visible. Limitaciones conocidas: los bots son aleatorios, comparten nombre con los puestos de siempre y no llevan marca de «bot».
...
- **Marca de «bot»** en los puestos y nombres propios para los bots.
...
- `README.md` desactualizado: todavía dice que el motor y el multijugador vienen después.
