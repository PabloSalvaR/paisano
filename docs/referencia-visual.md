...
- **Adornos de la mesa (`makeMate`, `makeFacon`, `board.js`):** todo procedural, sin archivos. Mate: calabaza con textura de piel (`gourdCanvas`), virola de acero, yerba de hojuelas (`yerbaCanvas` como color y relieve, más 1300 hojuelas 3D con `InstancedMesh`; la superficie conserva la montañita alta del lado opuesto a la bombilla y el pozo `PIT` junto a ella) y bombilla de caño curvo con el pico hacia afuera, aros dorados y filtro. Facón: cuchillo común de hoja **plana y fina** (extrusión sin bisel) con textura de acero opaco (`faconCanvas`: filo claro, manchas grises, grietas finas; sin óxido, sin canal), guarda con puntas redondas y mango con anillos de bronce. Con `?debug`, `window.__paisano.camera` y `.controls` permiten acercar la cámara a un adorno.
- Piezas: casa (perfil de casa extruido), estancia (sala + torre), camino (prisma), ladrón (torno + esfera).
